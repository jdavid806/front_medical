  ...
  <div class="content">
    <nav class="mb-3" aria-label="breadcrumb">
      <ol class="breadcrumb mb-0">
        ...
        <li class="breadcrumb-item"><a href="verPaciente?id=1" class="patientName" id="nameBradcumb">Cargando...</a></li>
                    ...
                    <div class="accordion-body">
                      <form id="filtroForm" class="d-flex align-items-end gap-2">
                        ...
                        <div class="flex-grow-1">
                          ...
                          <input class="form-control datetimepicker" id="filtroFecha" type="text" placeholder="dd/mm/yyyy"
                            data-options='{"disableMobile":true,"dateFormat":"d/m/Y"}' />
                        </div>

                        <!-- Botón para aplicar filtros -->
                        <button type="submit" class="btn btn-primary align-self-end">Buscar Nota</button>
                      </form>
                    </div>
                  </div>
                </div>
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    function filtrarDatos() {
      ...
      const datosFiltrados = historialData.filter((section) => {
        ...
        const coincideEnfermera = filtroEnfermera === "" || section.user_id.toString() === filtroEnfermera;

        // Convertir `created_at` a `DD/MM/YYYY` asegurando ceros iniciales
        const fechaObjeto = new Date(section.created_at);
        const dia = fechaObjeto.getDate().toString().padStart(2, "0");
        const mes = (fechaObjeto.getMonth() + 1).toString().padStart(2, "0"); // `getMonth()` devuelve 0-11
        ...
        if (filtroFecha.includes("-")) { // Si el input tiene formato YYYY-MM-DD
          ...
          filtroFechaFormateada = `${partes[2].padStart(2, "0")}/${partes[1].padStart(2, "0")}/${partes[0]}`;
        }
