  ...
  async function renderBoard() {
    ...
    Object.keys(groupedAdmissions).forEach(statusKey => {
      ...
      groupedAdmissions[statusKey].forEach(admission => {
        ...
        timeElement.textContent = `${admission.appointment_date} ${moment(admission.appointment_time, 'HH:mm:ss').format('HH:mm')}`;

        const patientInfo = document.createElement('p');
        patientInfo.textContent = `Paciente: ${admission.patient.first_name} ${admission.patient.last_name}, ${admission.patient.document_type} ${admission.patient.document_number}`;
