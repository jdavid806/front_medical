...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  formMedicamento.addEventListener("submit", async (event) => {
    ...
    const medicamentoData = {
      ...
      description:
        document.getElementById("descripcionMedicamento").value || null,
    ...
    try {
      if (editingProductId) {
        ...
        await inventoryService.storeProduct(medicamentoData).then((res) => {
          console.log(res);
        });
        await Swal.fire({
          title: "¡Éxito!",
  ...
  async function fetchMedicines() {
    try {
      ...
      medicines = response.data.filter(
        (product) => product.attributes.product_type_id == 4
      );
              ...
              return `
                    <tr class="table-row selectable-row">
                        <td class="align-middle ps-3">${
                          attributes.name || "Sin nombre"
                        }</td>
                        <td class="align-middle text-end">${
                          attributes.stock || "N/A"
                        }</td>
                        <td class="align-middle text-end">${
                          attributes.weight ? `${attributes.weight} g` : "N/A"
                        }</td>
                        <td class="align-middle text-end">${
                          attributes.capacity
                            ? `${attributes.capacity} ml`
                            : "N/A"
                        }</td>
                        <td class="align-middle text-end">${
                          attributes.concentration || "N/A"
                        }</td>
                        <td class="align-middle text-end">${formatDate(
                        ...
                        <td class="text-end align-middle pe-3">
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    <li><a class="dropdown-item editar-producto" href="#" data-id="${
                                      product.id
                                    }">Editar</a></li>
                                    <li><a class="dropdown-item eliminar-producto" href="#" data-id="${
                                      product.id
                                    }">Eliminar</a></li>
                                </ul>
                            </div>
    ...
    if (target.classList.contains("editar-producto")) {
      ...
      try {
        ...
        document.getElementById("tipoMedicamento").value =
          medicamento.medical_form_id || "";
        document.getElementById("stockMedicamento").value =
          medicamento.stock || "";
        document.getElementById("fechaCaducidad").value =
          medicamento.expiration_date || "";
        document.getElementById("loteMedicamento").value =
          medicamento.sanitary_registration || "";
        document.getElementById("descripcionMedicamento").value =
          medicamento.description || "";
        document.getElementById("pesoMedicamento").value =
          medicamento.weight || "";
        document.getElementById("capacidadMedicamento").value =
          medicamento.capacity || "";
        document.getElementById("precioCompraMedicamento").value =
          medicamento.purchase_price || "";
        document.getElementById("precioVentaMedicamento").value =
          medicamento.sale_price || "";

        document.getElementById("medicamentoPreview").src =
          medicamento.image || "https://via.placeholder.com/150";

        // Mostrar el modal manualmente
        const modal = new bootstrap.Modal(
          document.getElementById("modalNuevoMedicamento")
        );
        modal.show();
      } catch (error) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  verDetalleBtn.addEventListener("click", () => {
    ...
    document.getElementById("modalProductoId").textContent =
      selectedProduct.id || "-";
    ...
    document.getElementById("modalCodigoBarras").textContent =
      selectedProduct.barcode || "-";
    document.getElementById("modalStock").textContent =
      selectedProduct.stock || "-";
    document.getElementById("modalStockMin").textContent =
      selectedProduct.stock_min || "-";
    document.getElementById("modalStockMax").textContent =
      selectedProduct.stock_max || "-";
    document.getElementById("modalPrecioCompra").textContent = `$${
      selectedProduct.purchase_price || "-"
    }`;
    document.getElementById("modalPrecioVenta").textContent = `$${
      selectedProduct.sale_price || "-"
    }`;
    document.getElementById("modalEstado").textContent =
      selectedProduct.status || "-";
    document.getElementById("modalLaboratorio").textContent =
    ...
    document.getElementById("modalRegistroSanitario").textContent =
      selectedProduct.sanitary_registration || "-";
    document.getElementById("modalExpiracion").textContent =
      selectedProduct.expiration_date || "-";
    document.getElementById("modalConcentracion").textContent =
      selectedProduct.concentration || "-";
    document.getElementById("modalReceta").textContent =
      selectedProduct.requires_prescription ? "Sí" : "No";
  });
});
