...
export const useCompanyMutation = () => {
    ...
    const uploadFile = async (file: File, companyId: number, fileType: 'logo' | 'watermark'): Promise<UploadedFile> => {
            ...
            try {
                ...
                guardarArchivo(formData, true)
                    .then(async (response) => {
                        ...
                        const file_url = await getUrlImage(
    ...
    const guardarInformacionGeneral = async (formData: any, logoFile?: File, marcaAguaFile?: File, companyId?: number) => {
        try {
            setIsLoading(true);
            setError(null);
            setIsSuccess(false);
