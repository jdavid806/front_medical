...
<div>
  <table class="invoice-header">
    <tr>
      <td><h3 class="invoice-title">Factura de venta #${invoice.numeroFactura
            }</h3></td>
      <td class="invoice-date">Fecha: ${formatDate(invoice.fecha)}</td>
    </tr>
  </table>
  
  <p class="invoice-client"><strong>Cliente:</strong> ${invoice.cliente || "Sin cliente"
            }</p>
  <p class="invoice-client"><strong>Identificación:</strong> ${invoice.third_party?.document_type || "N/A"
            }-${invoice.third_party?.document_number || "-"}</p>
  <p class="invoice-client"><strong>Teléfono:</strong> ${invoice.third_party?.phone || "--"
            }</p>
  <p class="invoice-client"><strong>Correo:</strong> ${invoice.third_party?.email || "--"
            }</p>
