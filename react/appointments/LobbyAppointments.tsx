...
console.log("Appointments_LobbyAppointments",appointments);
  ...
  return (
    <div style={{ padding: "1rem" }}>
      ...
      {Object.entries(groupedAppointments).map(([stateLabel, stateAppointments]) => (
        <Panel key={stateLabel} header={stateLabel} toggleable collapsed className="mb-4">
          <ScrollPanel style={{ maxHeight: "400px" }}>
            ...
            <div style={{
              ...
              {stateAppointments?.map((appointment) => (
                <Card key={appointment.id} title={appointment.patientName} style={{
